...
app.post('/api/v1/register',function(req,res){
  ...
  var email = req.body.email;
  var password = req.body.password;
  pool.query('insert into public.users values($1,$2,$3)',[email,password,name],function(err,result){
    if (err){
        console.log(err);
      response.statusCode = "400";
      response.message = "failed";
...
app.post('/api/v1/article',function(req,res){
  ...
  pool.query("insert into public.article values($1,$2,$3,$4,$5)",[,email,title,date,content],function(err,result){
    if(err){
      response.statusCode = "400";
      response.message = "failed";
      response.data = {"message":"Something went wrong please try again."}
